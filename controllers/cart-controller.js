...
exports.addcart = async (req, res, next) => {
	try {
		...
		if (!product_id || !quantity || !sizeId) {
			return res
				.status(400)
				.json({ msg: "กรุณาระบุสินค้า, ขนาด และจำนวนให้ครบถ้วน" });
		}
...
exports.updateCartItem = async (req, res, next) => {
	try {
		...
		if (!cartItemId || !quantity) {
			return res
				.status(400)
				.json({ msg: "กรุณาระบุรายการสินค้าและจำนวนที่ต้องการแก้ไข" });
		}
		...
		if (isNaN(newQuantity) || newQuantity <= 0) {
			return res
				.status(400)
				.json({ msg: "จำนวนสินค้าต้องเป็นตัวเลขที่มากกว่า 0" });
		}

		console.log(
			"Request received at /update-cart-item with Clerk ID:",
			clerkID
		);
...
exports.removeCartItem = async (req, res, next) => {
	try {
		...
		console.log(
			"Request received at /remove-cart-item with Clerk ID:",
			clerkID
		);
